          ...
          child: Container(
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                color: selected ? accent : theme.typography.body?.color?.withOpacity(0.72),
              ),
            ),
          ),
        ),
                  ...
                  Expanded(
                    child: _Panel(
                      ...
                      children: overview.linkedNotes
                          .map((note) => Text(note.title, style: const TextStyle(fontSize: 12)))
                          .toList(),
                    ),
                  ),
                  ...
                  Expanded(
                    child: _Panel(
                      ...
                      children: overview.recentActivity
                          .map((activity) => Text(_activityText(activity), style: const TextStyle(fontSize: 12)))
                          .toList(),
                    ),
                  ),
                ],
    ...
    return _Card(
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Text('当前任务', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
              const Spacer(),
              Text('进度 ${task!.progress}%', style: const TextStyle(fontSize: 11)),
            ],
          ),
          ...
          if (task!.nextStep.isNotEmpty) ...[
            const SizedBox(height: 14),
            Text('下一步：${task!.nextStep}', style: const TextStyle(fontSize: 13)),
          ],
        ],
      ),
    );
                ...
                if (issue.nextInvestigationStep.isNotEmpty)
                  Flexible(
                    child: Text(
                      ...
                      style: TextStyle(
                        fontSize: 11,
                        color: FluentTheme.of(context).typography.body?.color?.withOpacity(0.55),
                      ),
                    ),
                  ),
              ],
        ...
        children: [
          ...
          if (children.isEmpty)
            Text(
              ...
              style: TextStyle(
                fontSize: 12,
                color: FluentTheme.of(context).typography.body?.color?.withOpacity(0.45),
              ),
            )
          else
